...
| src/codex.js | Codex 进程管理 + WS 代理 + codexChat |
| src/api.js | OpenAI 兼容 REST API |
| src/memory.js | 记忆系统（加载/保存/提取） |
| src/gemini.js | Gemini CLI 调用 |
| src/telegram/ | Telegram Bot（index/commands/qa/task） |
| src/patrol.js | 独立版自动巡检脚本 |
| config/system_prompt.md | AI 人设提示词 |
| .env | 环境变量（TG_TOKEN、TG_PROXY） |
| public/js/app.js | 前端 WebSocket 通信、线程管理、PWA |
| public/index.html | 页面入口 |
| public/css/style.css | 移动端深色主题样式 |
| qa/ | QA 自动巡查系统（独立子项目） |
| data_audit/ | 数据同步与审计工具集（独立子项目） |
...
- QA 和 data_audit 共用 `data_audit/data/dameng_mirror.db` 本地镜像库

## 数据审核工具集（data_audit/）

独立的数据同步与审计模块，从远程 DataAPI 拉取业务数据到本地 SQLite 镜像库：
...
# 完整性比对
python data_audit/sync/verify_data.py --type A
```
